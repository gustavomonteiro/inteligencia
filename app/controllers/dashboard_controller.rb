class DashboardController < ApplicationController
  def index
    ...
    # @top_5_distribuidores = GasStation.select('distribuidora, count(*)').group('distribuidora').where("lower(distribuidora) = 'branca'").order('count DESC').limit(5)
    @top_5_fornecedores = WeekCollection.select('fornecedor, count(*)').joins(:gas_station).group('fornecedor').where("lower(gas_stations.distribuidora) = 'branca'").order('count DESC').limit(5)
  end
end
